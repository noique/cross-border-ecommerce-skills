...
3. **预算合理性：** 日预算应基于毛利倒推。稳定期不可超过日均毛利的 50%；新品期（Month 1-2）可放宽至 80-100%（用广告换排名的冷启动阶段），但必须设定明确的降本时间节点
...
**图表 1：Campaign 预算分配饼图**
- 类型：`pie`
- 数据来源：从预算分配表提取各 Campaign 占比
- 插入位置：插入到预算分配章节末尾

**图表 2：ACoS 阶段目标折线图**
- 类型：`line`
- 数据来源：从分阶段目标提取新品期/增长期/稳定期 ACoS
- 插入位置：插入到阶段目标章节末尾

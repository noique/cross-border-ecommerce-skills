...
- **不允许：** 模型穿着未售产品、多角度拼图
- **服装类：** 必须有真人模特穿着（不可平铺）
- **AI 生成图片政策：** 官方主图政策（G1881）**无 AI 专项条款**，但主图站点标准明文要求「必须是实物的专业照片，禁 graphics / illustrations / mockups / placeholders」。照片级 AI 渲染属**灰色地带**——实操主图以**实拍为底**；AI 场景化仅用于副图 / lifestyle / 广告素材。Amazon 长期保留 reject / remove / alter / select 图片的权利（非「2026 新政」；「系统直接替换主图且不通知」的 2026 新政说法查无官方公告，勿写）。EU AI Act 透明条款 2026-08 起是否覆盖电商场景图，以最新执行指南为准
- **Alexa for Shopping（前身 Rufus，2026-05-13 品牌退役并入，推荐引擎沿用）可读图：** 清晰、无遮挡的产品图有助于其 VLM 理解商品目录并推荐；Amazon Lens 视觉搜索靠图像匹配召回，**多角度实拍质量的权重上升**（官方称视觉搜索量同比 +70% YoY）
...
3. **文案/标注** — 图上需要的文字（如有）
4. **AI 提示词** — 可直接用于主流 AI 图片生成模型的英文提示词
5. **拍摄指引** — 如果用真实拍摄的方案
...
### 第四步：主图 A/B 实验（Manage Your Experiments）
- 主图对点击率影响最大，上线后用 Seller Central 的 **Manage Your Experiments** 工具做主图 A/B 测试（品牌所有者 + 合规 ASIN 可用），以真实流量对比而非拍脑袋定稿
- 每次只测一个变量（主图角度 / 背景 / 是否带包装），实验期建议跑满工具推荐周期后再看显著性
- 胜出版本回填为正式主图，实验结论沉淀到品牌视觉规范复用到同系列产品

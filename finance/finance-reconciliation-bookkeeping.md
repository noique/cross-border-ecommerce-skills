---
name: finance-reconciliation-bookkeeping
description: Convert messy multi-channel payouts (Amazon FBA + Shopify + Stripe/PayPal) into an accrual-basis, audit-ready ledger and a true per-channel P&L — gross-to-net settlement decomposition into component journal entries, per-channel clearing accounts that must tie to ~0, a recommended chart of accounts (clearing / contra-revenue / platform+processing fees / sales-tax & VAT payable / FX gain-loss / deposits-in-transit / reserve / landed COGS), deposits-in-transit and account-level-reserve cutoff across month-end, a landed-cost COGS build-up with FIFO vs AVCO vs LIFO guidance plus the §448/§471/§263A accrual threshold and Form 3115 flag, realized vs unrealized FX treatment, a GM→CM2→CM3 channel margin waterfall, connector sizing (A2X vs Link My Books vs Synder → QuickBooks Online / Xero), and a checkable month-end close checklist with daily/weekly/monthly/quarterly cadence. Use when the user's books are wrong or unbuilt — net payout booked as revenue, fees dumped into COGS, collected sales tax counted as income, clearing accounts that never zero, settlements that won't tie to the bank. Triggers on "reconcile Amazon settlement", "month-end close", "clearing account won't zero", "payout doesn't match the bank", "A2X vs Link My Books", "accrual vs cash for FBA", "chart of accounts for ecommerce", "对账", "月结", "记账", "结算报表怎么入账", "payout 和银行对不上", "COGS 怎么算才对", "平台费能不能进 COGS", "汇兑损益怎么记", "多渠道真实 P&L". Pairs with /finance-landed-cost-unit-economics (this is its data-cleanliness layer — reconcile before you model) and /finance-fx-payout-optimizer (that skill optimizes the FX chain; this one books its results).
---

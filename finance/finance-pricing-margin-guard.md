---
name: finance-pricing-margin-guard
description: "Turn landed COGS into hard pricing guardrails: a per-SKU × per-channel contribution-margin waterfall with the full Amazon 2026 fee stack explicitly loaded (referral, FBA, inbound placement, Low-Inventory-Level Fee, fuel & inflation surcharge, storage/aged, returns, account-level reserve), a cost-plus floor / value ceiling / repricer floor-ceiling band anchored to contribution margin rather than price history, a multi-market VAT-inclusive shelf-price table for US/UK/DE/FR/IT/ES with true net realization after stripping destination VAT + Digital Services Fee + EPR, an FX sensitivity row plus refund-FX-mismatch flags, a \"who went underwater\" repricing trigger list after any fee/tariff/FX move, and a promo guardrail calculator (max safe discount %, break-even volume lift, BOGO feasibility, CM-floor check). Use when the user wants to set or defend a price, configure a repricer floor, price the same SKU into a new market, or verify a discount/BOGO/coupon can survive before launching it. Triggers on \"pricing guardrails\", \"repricer floor\", \"break-even price\", \"max safe discount\", \"can we afford this promo\", \"who went underwater after the fee hike\", \"VAT-inclusive pricing for EU\", \"定价\", \"毛利护栏\", \"这个价还能不能打\", \"促销打几折不亏\", \"BOGO 能不能做\", \"欧洲含税定价怎么定\", \"该涨价还是降价\", \"汇率波动吃掉多少毛利\", \"平台涨费后哪些款亏了\". Pairs with /finance-landed-cost-unit-economics (run it first for the landed COGS this skill consumes), /finance-tax-nexus-vat-diagnostic (all nexus / registration / filing judgments belong there, not here) and /dsite-conversion-ux (guardrail first, price presentation second)."
---

---
name: finance-tax-nexus-vat-diagnostic
description: Diagnose where a China→US/EU/UK/CA/AU seller must register, file, and pay — crossing sales channels × physical inventory location × trailing-12-month revenue by destination × category/HS code into a per-jurisdiction Registration Obligation Map (must-register-now / approaching-threshold / not-yet), a filing calendar including the $0 info returns people forget, a per-order tax-drag estimate as a % of ASP, an EPR stream checklist (Germany LUCID/VerpackG + WEEE + BattG, France UIN/ADEME + CITEO, PPWR verification, ESPR destruction ban), and a risk-flag list. Covers US state economic nexus (Wayfair $100k/200-txn and the states dropping the transaction count), FBA inventory as hard physical nexus, EU OSS €10k and IOSS ≤€150 with the mandatory EU intermediary, UK £135 LVCR deemed supplier, Canada CAD $30k GST/HST, Australia AUD $75k GST, the EU €3 interim customs duty vs the separate un-legislated ~€2 handling fee, and the China side — 9610 / 9710 / 9810 / 1210 customs models, export VAT rebate vs 综试区无票免税, and the 2025 platform tax-reporting regime. Use when the user is entering a market, expanding to Pan-EU FBA, or unsure whether marketplace withholding excuses them from registering and filing. Triggers on "do I need to register for VAT here", "nexus check", "does FBA inventory create nexus", "OSS vs IOSS", "EPR registration", "LUCID", "marketplace collected the tax — do I still file", "税务合规诊断", "要不要注册 VAT", "nexus 判定", "IOSS 要不要办", "EPR 要注册哪几条流", "平台代扣了还用报吗", "英国 £135", "欧盟 €3 关税", "出口退税还是无票免税", "税负占售价多少", "进哪个市场税务成本多高". Pairs with /finance-landed-cost-unit-economics (feeds it the tax-drag COGS inputs), /finance-pricing-margin-guard (tax drag → pricing), /finance-entity-structure-advisor (entity / income-tax / profit repatriation) and /brand-market-scan (market-entry cost side).
---
